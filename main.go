...
func parseConfig() error {
	data, err := ioutil.ReadFile("teach.yaml")
	if os.IsNotExist(err) {
		return fmt.Errorf("The configuration file (teach.yaml) doesn't exist.")
	} else if err != nil {
...
func run() error {
	...
	websocketProxyHandler := NewMultipleHostWebsocketReverseProxy()
	...
	// Specific routes
	r.Host(`{node:learn[0-9]{1,3}_[0-9]{1,3}_[0-9]{1,3}_[0-9]{1,3}}-{port:[0-9]*}.{tld:.*}`).HandlerFunc(proxyMultiplexer)
	r.Host(`{node:learn[0-9]{1,3}_[0-9]{1,3}_[0-9]{1,3}_[0-9]{1,3}}.{tld:.*}`).HandlerFunc(proxyMultiplexer)
